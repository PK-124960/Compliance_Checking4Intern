...
\begin{frame}{Research Questions \& Hypotheses}
    \begin{enumerate}
        ...
        \begin{itemize}
            \item[\textbf{H1}] Achieve $\geq$80\% deontic type classification accuracy
        \end{itemize}
...
\begin{frame}{Pipeline at Scale: The Data Funnel}
    ...
    \vspace{0.3cm}
    \textbf{Rule type distribution (post-audit gold)}: 308 obligation \quad|\quad 60 prohibition \quad|\quad 75 permission
\end{frame}
...
\begin{frame}{Why Not Direct NL $\rightarrow$ SHACL? (Ablation Study)}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{lcc}
            ...
            \textbf{NL $\rightarrow$ FOL $\rightarrow$ SHACL} & \textbf{100\%}\textsuperscript{$\dagger$} & \textbf{100\%} \\
            \bottomrule
        \end{tabular}
    \end{table}
    ...
    \vspace{0.2cm}
    \footnotesize\textsuperscript{$\dagger$}For the 351/443 rules that succeed on the FOL path. 92 rules (20.8\%) use the direct NL$\rightarrow$SHACL fallback.
\end{frame}
...
\begin{frame}{Pipeline Metrics with Confidence Intervals (D1-Grounded)}
    \begin{table}[]
        ...
        \begin{tabular}{llcc}
            ...
            \textbf{LLM Acc.} & Type accuracy vs.\ D1 gold (N=443) & \textbf{86.9\%} & ($\kappa$=0.709) \\
            \textbf{M3} & FOL formulas with semantic predicates & \textbf{100\%} & [100\%, 100\%] \\
            \textbf{M4} & Shape correctness (Corpus Adapter) & \textbf{F1=0.866} & (P=0.977, R=0.778) \\
...
\begin{frame}{Three-Annotator IRR Study (D1 Sample)}
    \begin{columns}
        ...
        \begin{itemize}
            \item IRR sample (N=50): \textbf{84.0\%}, $\kappa$ = 0.629
            \item Full D1 (N=443): \textbf{86.9\%}, $\kappa$ = \textbf{0.709}
        \end{itemize}
    \end{columns}

    \vspace{0.4cm}
    \begin{exampleblock}{Methodological Strength}
        The three-annotator IRR study on a stratified 50-item sample from D1 (1,663 sentences) establishes a robust, multi-annotator consensus as the gold standard for all LLM evaluation metrics. A subsequent disagreement audit corrected 12 gold label errors (epistemic ``may'', ``may not'' prohibitions), yielding the final post-audit distribution: 308~O, 75~P, 60~F.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Summary of Research Results}
    \begin{table}[]
        ...
        \begin{tabular}{llccc}
            ...
            \midrule
            RQ1 & LLM Accuracy (full D1, N=443) & $\geq$80\% & \textbf{86.9\%} & \textbf{Yes} \\
            RQ1 & Cohen's $\kappa$ (LLM vs.\ gold) & $\geq$0.60 & \textbf{0.709} & \textbf{Yes} \\
            RQ2 & FOL Expressiveness & 100\% & 100\% (0 SOL/HOL) & \textbf{Yes} \\
            RQ2 & FOL Pipeline Success & -- & 79.2\% (351/443) & -- \\
            RQ2 & SOL/HOL Needed & 0 & 0 & \textbf{Yes} \\
            RQ2 & M3 Predicate Quality & -- & \textbf{100\%} & \textbf{Yes} \\
            RQ3 & SHACL Translation & $\geq$95\% & \textbf{100\%} (443 shapes; 401 valid) & \textbf{Yes} \\
...
\begin{frame}{Research Contributions}
    \begin{enumerate}
        ...
        \vspace{0.2cm}
        \item \textbf{FOL Sufficiency Proof}: Empirical evidence that FOL with deontic operators is expressively sufficient for 100\% of institutional policy rules (79.2\% pipeline formalization, no SOL/HOL needed)
...
\begin{frame}{Limitations \& Future Work}
    ...
    \begin{itemize}
        \item Permission classification remains challenging (65.3\% recall on full D1 due to ``may'' ambiguity)
...
\begin{frame}{Conclusion}
    ...
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Classifies deontic rules} with 86.9\% accuracy ($\kappa$=0.709) across the full D1 corpus
        \item \textbf{Proves FOL sufficiency} for institutional policy formalization (79.2\% pipeline, M3 = 100\%)
...
\begin{frame}{Backup: Single Annotator Mitigation $\rightarrow$ 3-Annotator IRR}
    ...
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item Full D1 (N=443): Accuracy = \textbf{86.9\%}, $\kappa$ = \textbf{0.709}
            \item IRR sample (N=50): Accuracy = 84.0\%, $\kappa$ = 0.629
        \end{itemize}
    \end{columns}
...
\begin{frame}{Backup: M4 Inverted Cases --- Root Cause}
    ...
    \begin{enumerate}
        ...
        \item This is confirmed by the multi-LLM study: prohibition is the hardest class (Llama 3.1 8B gets only 40\% on prohibitions, N=5 in 50-item sample)
    \end{enumerate}
...
\begin{frame}{Backup: Deontic Type Confusion Matrix (Full D1, N=443)}
    \textbf{Q: ``Where exactly do the classification errors come from?''}
    ...
    \vspace{0.3cm}
    \textbf{Key error pathways:}
    \begin{itemize}
        \item \textbf{22 P$\rightarrow$O}: Permission ``may'' misread as obligation (``obligation bias'')
        \item \textbf{16 O$\rightarrow$F}: Consequence statements (``will not be allowed'') misread as prohibition
        \item \textbf{14 F$\rightarrow$O}: Compound sentences where obligation markers overshadow prohibition
    \end{itemize}

    \vspace{0.2cm}
    \footnotesize Post-audit gold labels (12 corrections applied: 10 P$\to$O, 2 P$\to$F).
\end{frame}

\begin{frame}{Backup: Per-Class Performance (Full D1, N=443)}
    \textbf{Q: ``What is the precision/recall tradeoff for each deontic type?''}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{lccccl}
            ...
            \midrule
            \textbf{Overall} & \textbf{443} & \textbf{385} & \multicolumn{3}{c}{\textbf{Acc=86.9\%, $\kappa$=0.709, Macro F1=0.810}} \\
            \bottomrule
        \end{tabular}
    \end{table}

    \vspace{0.3cm}
    \textbf{Notable findings:}
    \begin{itemize}
        \item \textbf{Permission precision paradox}: 98.0\% precision (near-zero false positives) but only 65.3\% recall --- the LLM is conservative, under-detecting permissions
        \item \textbf{Obligation is the strength}: F1=0.915, reflecting clear deontic markers (``must'', ``shall'')
        \item \textbf{Prohibition is the frontier}: lowest precision (69.7\%) due to boundary confusion with obligations
    \end{itemize}
\end{frame}
...
\begin{frame}{Backup: Baseline Classifier Comparison}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{lcccc}
            ...
            \midrule
            \textbf{Mistral 7B (Rule Detection)} & \textbf{93.81\%} & \textbf{97.53\%} & \textbf{95.18\%} & \textbf{96.34\%} \\
            \textbf{Mistral 7B (Type Classification)} & \textbf{86.9\%} & -- & -- & \textbf{Macro F1=0.810} \\
            \bottomrule
        \end{tabular}
    \end{table}
